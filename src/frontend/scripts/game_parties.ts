interface HoverableTextRule {
    word: string,
    action: string
}

interface HoverableText {
    name: string,
    hover: HoverableTextRule[]
}

interface GamePartyStyle {
    back_color: string,
    front_color: string,
    icon: string
}
...
interface PartyInfo {
    name: string,
    color: string,
    goal: string | HoverableText,
    rules: (string | HoverableText)[] | null,
    passive: (string | HoverableText)[] | null,
    bonus: (string | HoverableText)[] | null,
    special_moves: (string | HoverableText)[] | null,
    special_units: (string | HoverableText)[] | null,
    leader: (string | HoverableText | null),
    start: string | null,

    style: GamePartyStyle
}

class GameParty {
    info: PartyInfo;

    constructor(info: PartyInfo) {
        this.info = info;
    }

    private hoverableTextToHtml(input: HoverableText | string | null): string {
        if (!input)
            return "N/A";
        if (typeof input == "string")
            return input;
        let output = input.name;
        input.hover.forEach(h => output = output.replace(h.word, 
            `<a onmouseover="${h.action}" onmouseleave="hide_info();" class="party-info-a">${h.word}</a>`));
        return output;
    }

    private possibleHoverableListToHtml(input: (string | HoverableText)[] | null): string {
        if (!input)
            return "N/A";
        let response = "";
        input.forEach(part => {
            response += this.hoverableTextToHtml(part) + "<br>";
        });
        return response;
    }
    ...
    getAsHtml(): string {
        return `
            ...
            <hr>
            <h3>Objetivo: ${this.hoverableTextToHtml(this.info.goal)}</h3>
            <hr>

            <h3>Regras</h3>
            ${this.possibleHoverableListToHtml(this.info.rules)}

            <h3>Passivas</h3>
            ${this.possibleHoverableListToHtml(this.info.passive)}

            <h3>Bônus</h3>
            ${this.possibleHoverableListToHtml(this.info.bonus)}

            <h3>Movimentos Especiais</h3>
            ${this.possibleHoverableListToHtml(this.info.special_moves)}

            <h3>Tropas Especiais:</h3>
            ${this.possibleHoverableListToHtml(this.info.special_units)}

            <h3>Líder: ${this.hoverableTextToHtml(this.info.leader)}</h3>
            <h3>Local de Início: ${this.hoverableTextToHtml(this.info.start)}</h3>
        `;
    }
}
...
class GamePartyManager {
    ...
    constructPartyShowcase(id: string) {

        if (!this.parties.has(id)) {
            console.error(`Party '{id}' not found when trying to construct its showcase!`);
            return;
        }

        return this.parties.get(id)?.getAsHtml();

    }

    parties: Map<string, GameParty> = new Map([
        ['ANQ', new GameParty({
            name: "Anarquismo",
            color: "#000000",
            ...
            leader: null,
            start: null,
            style: {back_color: "#000000", front_color: "#ffffff", icon: "img/icons/anq.svg"}
        })],
        ['ISI', new GameParty({
            name: "ISIS",
            color: "#555555",
            ...
            start: "Oriente Médio",
            style: {back_color: "#aaaaaa", front_color: "#000000", icon: "img/icons/isis.svg"}
        })],
        ['FDL', new GameParty({
            name: "Feudalismo",
            color: "#40ff19",
            ...
            leader: "Carlos Magno",
            start: "França",
            style: {back_color: "#4cc963", front_color: "#000000", icon: "img/icons/feudalismo.svg"}
        })],
        ['VKG', new GameParty({
            name: "Vikings",
            color: "#ffcc66",
            ...
            style: {back_color: "#4cc993", front_color: "#000000", icon: "img/icons/vikings.svg"}
        })],
        ['ZMB', new GameParty({
            name: "Zumbis",
            color: "#116600",
            ...
            leader: "Dr. Zumbão",
            start: null,
            style: {back_color: "#4e9335", front_color: "#000000", icon: "img/icons/zombie.svg"}
        })],
        ['ONU', new GameParty({
            name: "ONU",
            color: "#80bfff",
            ...
            leader: "Barry",
            start: "New York",
            style: {back_color: "#23cec3", front_color: "#000000", icon: "img/icons/onu.svg"}
        })],
        ['PRT', new GameParty({
            name: "Piratas",
            color: "#0022cc",
            ...
            style: {back_color: "#7c3bdd", front_color: "#000000", icon: "img/icons/pirate.svg"}
        })],
        ['CAT', new GameParty({
            ...
            leader: "Papa",
            start: "Itália (França)",
            style: {back_color: "#d6d6d6", front_color: "#000000", icon: "img/icons/igreja.svg"}
        })],
        ['NZI', new GameParty({
            name: "Nazismo",
            color: "#ffffff",
            ...
            leader: "Adolf Hitler",
            start: "Alemanha",
            style: {back_color: "#ffffff", front_color: "#000000", icon: "img/icons/nazi.svg"}
        })],
        ['FSC', new GameParty({
            name: "Fascismo",
            color: "#ff8080",
            ...
            start: null,
            style: {back_color: "#d85136", front_color: "#000000", icon: "img/icons/fascism.svg"}
        })],
        ['JEY', new GameParty({
            name: "Joeyismo",
            color: "#800000",
            ...
            leader: "Joey",
            start: "Brasil",
            style: {back_color: "#d8366c", front_color: "#000000", icon: "img/icons/joeyismo.svg"}
        })],
        ['SQR', new GameParty({
            name: "SPQR",
            color: "#661100",
            ...
            start: "Itália (França)",
            style: {back_color: "#d6262f", front_color: "#000000", icon: "img/icons/spqr.svg"}
        })],
        ['RST', new GameParty({
            name: "Resistência",
            color: "#ff3333",
            ...
            leader: null,
            start: null,
            style: {back_color: "#a50e16", front_color: "#ffffff", icon: "img/icons/resistencia.svg"}
        })],
        ['SCL', new GameParty({
            name: "Socialismo",
            color: "#ff0000",
            ...
            leader: "Karl Marx",
            start: "Moscou",
            style: {back_color: "#f40c17", front_color: "#000000", icon: "img/icons/socialismo.svg"}
        })],
        ['CTL', new GameParty({
            name: "Capitalismo",
            color: "#ffff33",
            ...
            leader: "Henry Ford",
            start: "Califórnia",
            style: {back_color: "#efcc2f", front_color: "#000000", icon: "img/icons/capitalism.svg"}
        })],
        ['PTN', new GameParty({
            name: "Puritanismo",
            color: "#e69900",
            ...
            start: "Inglaterra",
            style: {back_color: "#4043f7", front_color: "#000000", icon: "img/icons/puritanism.svg"}
        })]
    ]);

    parties_dict: Map<string, string> = new Map([
...
export { GamePartyManager, GameParty }